 ...
 * @NFT_GOTO: jump to a chain without pushing the current chain on the jump
 * stack
 ...
 * @NFT_MSG_GETRULE_RESET: get rules and reset stateful expressions (enum
 * nft_obj_attributes)
 ...
 * @NFT_MSG_GETSETELEM_RESET: get set elements and reset attached stateful
 * expressions (enum nft_set_elem_attributes)
 */
 ...
 * @NFTA_CHAIN_HOOK: hook specification for basechains (NLA_NESTED:
 * nft_hook_attributes)
 ...
 * (NLA_U32)
 * @NFTA_RULE_CHAIN_ID: add the rule to chain by ID, alternative to
 * @NFTA_RULE_CHAIN (NLA_U32)
 */
 ...
 */
enum nft_set_desc_attributes { NFTA_SET_DESC_UNSPEC, NFTA_SET_DESC_SIZE, NFTA_SET_DESC_CONCAT, __NFTA_SET_DESC_MAX };
 ...
 */
enum nft_set_field_attributes { NFTA_SET_FIELD_UNSPEC, NFTA_SET_FIELD_LEN, __NFTA_SET_FIELD_MAX };
...
/**
 * enum nft_set_elem_list_attributes - nf_tables set element list netlink
 * attributes
 *
 * @NFTA_SET_ELEM_LIST_TABLE: table of the set to be changed (NLA_STRING)
 * @NFTA_SET_ELEM_LIST_SET: name of the set to be changed (NLA_STRING)
 * @NFTA_SET_ELEM_LIST_ELEMENTS: list of set elements (NLA_NESTED:
 * nft_set_elem_attributes)
 * @NFTA_SET_ELEM_LIST_SET_ID: uniquely identifies a set in a transaction
 * (NLA_U32)
 */
 ...
 */
enum nft_expr_attributes { NFTA_EXPR_UNSPEC, NFTA_EXPR_NAME, NFTA_EXPR_DATA, __NFTA_EXPR_MAX };
#define NFTA_EXPR_MAX (__NFTA_EXPR_MAX - 1)

/**
 * enum nft_immediate_attributes - nf_tables immediate expression netlink
 * attributes
 *
 ...
 */
enum nft_immediate_attributes { NFTA_IMMEDIATE_UNSPEC, NFTA_IMMEDIATE_DREG, NFTA_IMMEDIATE_DATA, __NFTA_IMMEDIATE_MAX };
...
/**
 * enum nft_byteorder_attributes - nf_tables byteorder expression netlink
 * attributes
 *
 ...
 */
enum nft_cmp_attributes { NFTA_CMP_UNSPEC, NFTA_CMP_SREG, NFTA_CMP_OP, NFTA_CMP_DATA, __NFTA_CMP_MAX };
 ...
 * @NFTA_DYNSET_SET_ID: uniquely identifier of the set in the transaction
 * (NLA_U32)
 ...
 * @NFT_META_PROTOCOL: packet ethertype protocol (skb->protocol), invalid in
 * OUTPUT
 ...
 */
enum nft_rt_keys { NFT_RT_CLASSID, NFT_RT_NEXTHOP4, NFT_RT_NEXTHOP6, NFT_RT_TCPMSS, NFT_RT_XFRM, __NFT_RT_MAX };
 ...
 */
enum nft_meta_attributes { NFTA_META_UNSPEC, NFTA_META_DREG, NFTA_META_KEY, NFTA_META_SREG, __NFTA_META_MAX };
 ...
 */
enum nft_rt_attributes { NFTA_RT_UNSPEC, NFTA_RT_DREG, NFTA_RT_KEY, __NFTA_RT_MAX };
...
/**
 * enum nft_connlimit_attributes - nf_tables connlimit expression netlink
 * attributes
 *
 ...
 */
enum nft_last_attributes { NFTA_LAST_UNSPEC, NFTA_LAST_SET, NFTA_LAST_MSECS, NFTA_LAST_PAD, __NFTA_LAST_MAX };
 ...
 */
enum nft_reject_attributes { NFTA_REJECT_UNSPEC, NFTA_REJECT_TYPE, NFTA_REJECT_ICMP_CODE, __NFTA_REJECT_MAX };
 ...
 * @NFTA_NAT_REG_ADDR_MIN: source register of address range start (NLA_U32:
 * nft_registers)
 * @NFTA_NAT_REG_ADDR_MAX: source register of address range end (NLA_U32:
 * nft_registers)
 * @NFTA_NAT_REG_PROTO_MIN: source register of proto range start (NLA_U32:
 * nft_registers)
 * @NFTA_NAT_REG_PROTO_MAX: source register of proto range end (NLA_U32:
 * nft_registers)
 * @NFTA_NAT_FLAGS: NAT flags (see NF_NAT_RANGE_* in linux/netfilter/nf_nat.h)
 * (NLA_U32)
 */
 ...
 *
 * @NFTA_MASQ_FLAGS: NAT flags (see NF_NAT_RANGE_* in linux/netfilter/nf_nat.h)
 * (NLA_U32)
 * @NFTA_MASQ_REG_PROTO_MIN: source register of proto range start (NLA_U32:
 * nft_registers)
 * @NFTA_MASQ_REG_PROTO_MAX: source register of proto range end (NLA_U32:
 * nft_registers)
 */
 ...
 *
 * @NFTA_REDIR_REG_PROTO_MIN: source register of proto range start (NLA_U32:
 * nft_registers)
 * @NFTA_REDIR_REG_PROTO_MAX: source register of proto range end (NLA_U32:
 * nft_registers)
 * @NFTA_REDIR_FLAGS: NAT flags (see NF_NAT_RANGE_* in linux/netfilter/nf_nat.h)
 * (NLA_U32)
 */
 ...
 * nft_register)
 */
enum nft_dup_attributes { NFTA_DUP_UNSPEC, NFTA_DUP_SREG_ADDR, NFTA_DUP_SREG_DEV, __NFTA_DUP_MAX };
 ...
 * nfproto)
 */
enum nft_fwd_attributes { NFTA_FWD_UNSPEC, NFTA_FWD_SREG_DEV, NFTA_FWD_SREG_ADDR, NFTA_FWD_NFPROTO, __NFTA_FWD_MAX };
 ...
 */
enum nft_gen_attributes { NFTA_GEN_UNSPEC, NFTA_GEN_ID, NFTA_GEN_PROC_PID, NFTA_GEN_PROC_NAME, __NFTA_GEN_MAX };
 ...
 * to the packet data.
 */
enum nft_fib_attributes { NFTA_FIB_UNSPEC, NFTA_FIB_DREG, NFTA_FIB_RESULT, NFTA_FIB_FLAGS, __NFTA_FIB_MAX };
 ...
 *
 * @NFTA_FLOWTABLE_TABLE: name of the table containing the expression
 * (NLA_STRING)
...
/**
 * enum nft_flowtable_hook_attributes - nf_tables flow table hook netlink
 * attributes
 *
 * @NFTA_FLOWTABLE_HOOK_NUM: netfilter hook number (NLA_U32)
 * @NFTA_FLOWTABLE_HOOK_PRIORITY: netfilter hook priority (NLA_U32)
 * @NFTA_FLOWTABLE_HOOK_DEVS: input devices this flow table is bound to
 * (NLA_NESTED)
 */
 ...
 */
enum nft_devices_attributes { NFTA_DEVICE_UNSPEC, NFTA_DEVICE_NAME, NFTA_DEVICE_PREFIX, __NFTA_DEVICE_MAX };
...
enum nft_tunnel_mode { NFT_TUNNEL_MODE_NONE, NFT_TUNNEL_MODE_RX, NFT_TUNNEL_MODE_TX, __NFT_TUNNEL_MODE_MAX };
