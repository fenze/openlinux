 ...
 * enclave. A NE CPU pool has to be set before calling this function. The pool
 * can be set after the NE driver load, using
 ...
 * *  -1				- There was a failure in the ioctl
 * logic. On failure, errno is set to:
 ...
 * *  -1				- There was a failure in the ioctl
 * logic. On failure, errno is set to:
 * * EFAULT				- copy_from_user() / copy_to_user()
 * failure.
 * * ENOMEM				- Memory allocation failure for internal
 *					  bookkeeping variables.
 * * EIO				- Current task mm is not the same as the
 * one that created the enclave.
 ...
 * * NE_ERR_VCPU_NOT_IN_CPU_POOL	- The provided vCPU is not available in
 * the NE CPU pool.
 * * NE_ERR_VCPU_INVALID_CPU_CORE	- The core id of the provided vCPU is
 * invalid or out of range.
 * * NE_ERR_NOT_IN_INIT_STATE		- The enclave is not in init state
 *					  (init = before being started).
 * * NE_ERR_INVALID_VCPU		- The provided vCPU is not in the
 * available CPUs range.
 * * Error codes from the NE PCI device request.
 */
 ...
 * * NE_ERR_INVALID_FLAG_VALUE		- The value of the provided flag is
 * invalid.
 ...
 *				    pages from different NUMA nodes than the
 * CPUs.
 */
