...
/* Deprecated: use struct bpf_lpm_trie_key_u8 (when the "data" member is needed
 * for byte access) or struct bpf_lpm_trie_key_hdr (when using an alternative
 * type for the trailing flexible array member) instead.
 */
...
enum {
	...
	/* When user space page faults in bpf_arena send SIGSEGV instead of
	   inserting new page */
...
union bpf_attr {
	...
	struct { /* anonymous struct used by BPF_PROG_LOAD command */
		...
		/* output: actual total log contents size (including termintaing
		 * zero). It could be both larger than original log_size (if log
		 * was truncated), or smaller (if log buffer wasn't filled
		 * completely).
		 */
		__u32 log_true_size;
 ...
 * the packet associated to *skb*, at *offset*. The *flags* are a combination of
 ...
 * the option resides and the name *optname* of the option must be specified,
 * see **setsockopt(2)** for more information. The option value of length
 ...
 * associated to *bpf_socket*, which must be a full socket. The *level* at which
 * the option resides and the name *optname* of the option must be specified,
 * see **getsockopt(2)** for more information. The retrieved value is stored in
 * the structure pointed by *opval* and of length *optlen*.
 *
 * 		*bpf_socket* should be one of the following:
 *
 * 		* **struct bpf_sock_ops** for **BPF_PROG_TYPE_SOCK_OPS**.
 ...
 * lookup. This option should not be used with BPF_FIB_LOOKUP_DIRECT, as it only
 * has meaning for full lookups.
 *
 ...
 * corresponding IP/TCP headers, *iph* and *th*, on the listening socket in
 * *sk*.
 *
 *		*iph* points to the start of the IPv4 or IPv6 header, while
 *		*iph_len* contains **sizeof**\ (**struct iphdr**) or
 ...
 * u64 size) Description Write raw *data* blob into a special BPF perf event
 * held by *map* of type **BPF_MAP_TYPE_PERF_EVENT_ARRAY**. This perf event must
 * have the following attributes: **PERF_SAMPLE_RAW** as **sample_type**,
 ...
 * retrieve the branch records (**struct perf_branch_entry**) associated to
 * *ctx* and store it in the buffer pointed by *buf* up to size *size* bytes.
 *	Return
 ...
 * kernel memory fails, the string for **%s** will be an empty string, and the
 * ip address for **%p{i,I}{4,6}** will be 0. Not returning error to bpf program
 * is consistent with what **bpf_trace_printk**\ () does for now. Return 0 on
 ...
 * buffer. Note: the user stack will only be populated if the *task* is the
 * current task; all other tasks will return -EOPNOTSUPP. To achieve this, the
 * helper needs *task*, which is a valid pointer to **struct task_struct**. To
 * store the stacktrace, the bpf program provides *buf* with a nonnegative
 * *size*.
 *
 *		The last argument, *flags*, holds the number of stack frames to
 *		skip (from 0 to 255), masked with
 ...
 * used to look up vmlinux BTF type ids. Traversing the data structure using
 * BTF, the type information and values are stored in the first *str_size* - 1
 * bytes of *str*.  Safe copy of the pointer data is carried out to avoid kernel
 ...
 * cpu and will not repeat unless another bpf_timer_start() is made. In such
 * case the next invocation can migrate to a different cpu. Since struct
 * bpf_timer is a field inside map element the map owns the timer. The
 ...
 * zero all timers in a map are cancelled and corresponding program's refcnts
 * are decremented. This is done to make sure that Ctrl-C of a user process
 * doesn't leave any timers running. If map is pinned in bpffs the callback_fn
 * can re-arm itself indefinitely. bpf_map_update/delete_elem() helpers and user
 * space sys_bpf commands cancel and free the timer in the given map element.
 *		The map can contain timers that invoke callback_fn-s from
 * different programs. The same callback_fn can serve different timers from
 ...
 * *res) Description Get the address of a kernel symbol, returned in *res*.
 * *res* is set to 0 if the symbol is not found. Return On success, zero. On
 ...
 * *callback_ctx, u64 flags) Description Find vma of *task* that contains
 * *addr*, call *callback_fn* function with *task*, *vma*, and *callback_ctx*.
 *		The *callback_fn* should be a static function and
 *		the *callback_ctx* should be a pointer to the stack.
 *		The *flags* is used to control certain aspects of the helper.
 *		Currently, the *flags* must be 0.
 ...
 * by the hooks where BPF program's return value is returned to the userspace
 * via errno. Return The BPF program's return value.
 *
 ...
 * len) Description Store *len* bytes from buffer *buf* into the frame
 ...
 *	Description
 *		Exchange kptr at pointer *dst* with *ptr*, and return the old
 * value. *dst* can be map value or local kptr. *ptr* can be NULL, otherwise it
 * must be a referenced pointer which will be released when this helper is
 * called. Return The old value of kptr (which can be NULL). The returned
 * pointer if not NULL, is a reference which must be released using its
...
struct bpf_sk_lookup {
	...
	__u32 ingress_ifindex; /* The arriving interface. Determined by
				  inet_iif. */
};
