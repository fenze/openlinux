...
#define MD_DISK_CLUSTER_ADD                         \
	4 /* Initiate a disk add across the cluster \
	   * For clustered enviroments only.        \
	   */
#define MD_DISK_CANDIDATE                                   \
	5 /* disk is added as spare (local) until confirmed \
	   * For clustered enviroments only.                \
	   */
#define MD_DISK_FAILFAST                              \
	10 /* Send REQ_FAILFAST if there are multiple \
...
struct mdp_superblock_1 {
	...
	__le32 raid_disks;
	union {
		__le32 bitmap_offset; /* sectors after start of superblock that
				       * bitmap starts NOTE: signed, so bitmap
				       * can be before superblock only
				       * meaningful of feature_map[0] is set.
				       */

		/* only meaningful when feature_map[MD_FEATURE_PPL] is set */
		struct {
			__le16 offset; /* sectors from start of superblock that
					  ppl starts (signed) */
...
#define FailFast1    2		   /* Should avoid retries and fixups and just fail */
	...
	__le64 resync_offset; /* data before this offset (from data_offset)
				 known to be in sync */
	 ...
	 * into the 'roles' value.  If a device is spare or faulty, then it
	 * doesn't have a meaningful role.
	 */
	__le16 dev_roles[]; /* role in array, or 0xffff for a spare, or 0xfffe
			       for faulty */
};
...
#define MD_FEATURE_RECOVERY_BITMAP                                  \
	128			      /* recovery that is happening \
				       * is guided by bitmap.       \
				       */
...
struct r5l_meta_block {
	...
	__le64 seq;
	__le64 position; /* sector, start from rdev->data_offset, current
			    position */
