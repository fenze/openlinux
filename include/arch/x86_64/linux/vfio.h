 ...
 * Unimplemented IRQ types return a count of zero.
 */
enum { VFIO_AP_REQ_IRQ_INDEX, VFIO_AP_CFG_CHG_IRQ_INDEX, VFIO_AP_NUM_IRQS };
 ...
 * ownership for devices within the same DMA isolation context.  In this case
 ...
 * supported only for CDX devices. When the device's BUS MASTER setting is
 * configured as CLEAR, it will result in blocking all incoming DMA requests
 * from the device. On the other hand, configuring the device's BUS MASTER
 * setting as SET (enable) will grant the device the capability to perform DMA
 * to the host memory.
 */
...
/**
 * VFIO_IOMMU_SPAPR_UNREGISTER_MEMORY - _IOW(VFIO_TYPE, VFIO_BASE + 18, struct
 * vfio_iommu_spapr_register_memory)
 *
