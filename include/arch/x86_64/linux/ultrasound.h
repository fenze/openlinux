 ...
 *		_GUS_NUMVOICES	- Sets max. number of concurrent voices
 * (P1=14-31, default 16) _GUS_VOICESAMPLE- ************ OBSOLETE *************
 *		_GUS_VOICEON	- Starts voice (P1=voice mode)
 *		_GUS_VOICEOFF	- Stops voice (no parameters)
 *		_GUS_VOICEFADE	- Stops the voice smoothly.
 *		_GUS_VOICEMODE	- Alters the voice mode, don't start or stop
 * voice (P1=voice mode) _GUS_VOICEBALA	- Sets voice balance (P1, 0=left,
 * 7=middle and 15=right, default 7) _GUS_VOICEFREQ	- Sets voice (sample)
 * playback frequency (P1=Hz) _GUS_VOICEVOL	- Sets voice volume (P1=volume,
 * 0xfff=max, 0xeff=half, 0x000=off) _GUS_VOICEVOL2	- Sets voice volume
 * (P1=volume, 0xfff=max, 0xeff=half, 0x000=off) (Like GUS_VOICEVOL but doesn't
 * change the hw volume. It just updates volume in the voice table).
 *
 *		_GUS_RAMPRANGE	- Sets limits for volume ramping (P1=low volume,
 * P2=high volume) _GUS_RAMPRATE	- Sets the speed for volume ramping
 * (P1=scale, P2=rate) _GUS_RAMPMODE	- Sets the volume ramping mode
 * (P1=ramping mode) _GUS_RAMPON	- Starts volume ramping (no parameters)
