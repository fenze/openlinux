 ...
 *
 * Note. This is alternative to the structure 'struct file_kattr'/'struct
 * fsxattr'. As this structure is passed to/from userspace with its size, this
 * can be versioned based on the size.
 */
struct file_attr {
 ...
 *			walk_end == end (address tags cleared) informs that the
 * scan completed on entire range.
...
enum procmap_query_flags {
	 ...
	 * only VMAs satisfying specified subset of permissions. E.g.,
	 * specifying PROCMAP_QUERY_VMA_READABLE only will return both readable
	 * and read/write VMAs, while having PROCMAP_QUERY_VMA_READABLE |
	 * PROCMAP_QUERY_VMA_WRITABLE will only return read/write VMAs, though
	 * both executable/non-executable and private/shared will be ignored.
	 *
	 * PROCMAP_QUERY_VMA_* flags are also returned in
	 * procmap_query.vma_flags field to specify actual VMA permissions.
	 */
	 ...
	 * is returned. With PROCMAP_QUERY_COVERING_OR_NEXT_VMA flag set,
	 * closest VMA with vma_start > addr will be returned if no covering VMA
	 * is found.
	 *
	 * PROCMAP_QUERY_FILE_BACKED_VMA instructs query to consider only VMAs
	 * that have file backing. Can be combined with
	 * PROCMAP_QUERY_COVERING_OR_NEXT_VMA to iterate all VMAs with file
	 * backing.
	 */
 ...
 * right of the field:
 *   - "in", input argument, user has to provide this value, kernel doesn't
 * modify it;
...
struct procmap_query {
	 ...
	 */
	__u64 query_addr; /* in */
	/* VMA starting (inclusive) and ending (exclusive) address, if VMA is
	 * found. */
	__u64 vma_start; /* out */
	__u64 vma_end;	 /* out */
	...
	__u64 inode; /* out */
	/* Backing file's device major/minor number, or zero, if VMA has no
	 * backing file. */
	__u32 dev_major; /* out */
	__u32 dev_minor; /* out */
	/*
