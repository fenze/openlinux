...
/*
 * Special __inline__ ref key which stores the id of the subvolume which
 * originally created the extent. This subvolume owns the extent permanently
 * from the perspective of simple quotas. Needed to know which subvolume to free
 * quota usage from when the extent is deleted.
 *
 * Stored as an __inline__ ref rather to avoid wasting space on a separate item
 * on top of the existing extent item. However, unlike the other __inline__
 * refs, there is one one owner ref per extent rather than one per extent.
 *
 * Because of this, it goes at the front of the list of __inline__ refs, and
 * thus must have a lower type value than any other __inline__ ref type (to
 * satisfy the disk format rule that __inline__ refs have non-decreasing type).
 */
...
#define BTRFS_INODE_FLAG_MASK                                                                                        \
	(BTRFS_INODE_NODATASUM | BTRFS_INODE_NODATACOW | BTRFS_INODE_READONLY | BTRFS_INODE_NOCOMPRESS |             \
	 BTRFS_INODE_PREALLOC | BTRFS_INODE_SYNC | BTRFS_INODE_IMMUTABLE | BTRFS_INODE_APPEND | BTRFS_INODE_NODUMP | \
...
#define BTRFS_BLOCK_GROUP_PROFILE_MASK                                                                               \
	(BTRFS_BLOCK_GROUP_RAID0 | BTRFS_BLOCK_GROUP_RAID1 | BTRFS_BLOCK_GROUP_RAID1C3 | BTRFS_BLOCK_GROUP_RAID1C4 | \
	 BTRFS_BLOCK_GROUP_RAID5 | BTRFS_BLOCK_GROUP_RAID6 | BTRFS_BLOCK_GROUP_DUP | BTRFS_BLOCK_GROUP_RAID10)
...
#define BTRFS_QGROUP_STATUS_FLAGS_MASK                                                                           \
	(BTRFS_QGROUP_STATUS_FLAG_ON | BTRFS_QGROUP_STATUS_FLAG_RESCAN | BTRFS_QGROUP_STATUS_FLAG_INCONSISTENT | \
...
struct btrfs_qgroup_status_item {
	...
	/*
	 * The generation when quotas were last enabled. Used by simple quotas
	 * to avoid decrementing when freeing an extent that was written before
	 * enable.
	 *
