...
struct pidfd_info {
	 ...
	 * the time it is received, as the target process might have exited as
	 * soon as the IOCTL was processed, and there is no way to avoid that.
	 * However, it is guaranteed that if the call was successful, then the
	 * information was correct and referred to the intended process at the
