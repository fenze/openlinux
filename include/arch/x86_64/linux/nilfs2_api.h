...
#define NILFS_CPINFO_FNS(flag, name)                                        \
	static __inline__ int nilfs_cpinfo_##name(                          \
		const struct nilfs_cpinfo *cpinfo)                          \
	{                                                                   \
...
#define NILFS_SUINFO_FNS(flag, name)                                 \
	static __inline__ int nilfs_suinfo_##name(                   \
...
#define NILFS_SUINFO_UPDATE_FNS(flag, name)                             \
	static __inline__ void nilfs_suinfo_update_set_##name(          \
		struct nilfs_suinfo_update *sup)                        \
	{                                                               \
		sup->sup_flags |= 1UL << NILFS_SUINFO_UPDATE_##flag;    \
	}                                                               \
	static __inline__ void nilfs_suinfo_update_clear_##name(        \
		struct nilfs_suinfo_update *sup)                        \
	{                                                               \
		sup->sup_flags &= ~(1UL << NILFS_SUINFO_UPDATE_##flag); \
	}                                                               \
	static __inline__ int nilfs_suinfo_update_##name(               \
		const struct nilfs_suinfo_update *sup)                  \
	{                                                               \
		return !!(sup->sup_flags &                              \
			  (1UL << NILFS_SUINFO_UPDATE_##flag));         \
	}
