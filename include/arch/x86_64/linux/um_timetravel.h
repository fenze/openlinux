...
enum um_timetravel_ops {
	 ...
	 * process this message and later	send a %UM_TIMETRAVEL_RUN
	 * message when the host can run again. (host -> calendar)
	 */
 ...
 * aligned (i.e. aligned to their size).
 */
 ...
 * client using the shared memory, and can be set only by the client after it
 * gets the fd memory.
 *
 ...
 *	per connection first mmap the header and take the length (@len) to remap
 * the entire size. This is done in order to support dynamic struct size letting
 * number of clients be dynamic based on controller support.
 ...
 * @free_until, so it must remain smaller than @free_until.
