...
#define ETHTOOL_GSET                                   \
	0x00000001 /* DEPRECATED, Get settings.        \
		    * Please use ETHTOOL_GLINKSETTINGS \
		    */
#define ETHTOOL_SSET                                                    \
	0x00000002		    /* DEPRECATED, Set settings.        \
...
enum {
	 ...
	 * Container). The format of these GTP packets includes TEID and QFI. In
	 * 5G communication using UPF (User Plane Function), data communication
	 * with this extended header is performed.
	 */
	GTPU_EH_V4_FLOW = 0x19, /* hash only */
	GTPU_EH_V6_FLOW = 0x1a, /* hash only */

	/* Use for GTP-U IPv4 and v6 PSC (PDU Session Container) extended
	 * headers. This differs from GTPU_EH_V(4|6)_FLOW in that it is
	 * distinguished by UL/DL included in the PSC. There are differences in
	 * the data included based on Downlink/Uplink, and can be used to
	 * distinguish packets. The functions described so far are useful when
	 * you want to handle communication from the mobile network in UPF, PGW,
	 * etc.
	 */
