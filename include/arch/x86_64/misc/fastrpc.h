...
#define FASTRPC_IOCTL_INIT_CREATE_STATIC \
	_IOWR('R', 9, struct fastrpc_init_create_static)
 ...
 * WRITEBACK. The driver is responsible for cache maintenance when passed the
 * buffer to FastRPC calls. Same virtual address will be assigned for subsequent
 * FastRPC calls.
 ...
 * @FASTRPC_MAP_FD_DELAYED: Mapping delayed until user call HAP_mmap() and
 * HAP_munmap() functions on DSP. It is useful to map a buffer with cache modes
