...
/**
 * struct snd_firewire_event_ff400_message - the container for message from
 * Fireface 400 when operating hardware knob.
 *
 ...
 * @messages.tstamp: The isochronous cycle at which the request subaction of
 * asynchronous transaction was sent to deliver the message. It has 16 bit
 * unsigned integer value. The higher 3 bits of value expresses the lower three
 * bits of second field in the format of CYCLE_TIME, up to 7. The rest 13 bits
 * expresses cycle field up to 7999.
 *
 * The structure expresses message transmitted by Fireface 400 when operating
 * hardware knob.
 */
...
/*
 * In below MOTU models, software is allowed to control their DSP by accessing
 * to registers.
 *  - 828mk2
 *  - 896hd
 *  - Traveler
 ...
 *  - Audio Express
 *
 * On the other hand, the status of DSP is split into specific messages included
 * in the sequence of isochronous packet. ALSA firewire-motu driver gathers the
 * messages and allow userspace applications to read it via ioctl. In 828mk2,
 * 896hd, and Traveler, hardware meter for all of physical inputs are put into
 * the message, while one pair of physical outputs is selected. The selection is
 * done by LSB one byte in asynchronous write quadlet transaction to
 * 0x'ffff'f000'0b2c.
 *
 * I note that V3HD/V4HD uses asynchronous transaction for the purpose. The
 * destination address is registered to 0x'ffff'f000'0b38 and '0b3c by
 * asynchronous write quadlet request. The size of message differs between 23
 * and 51 quadlets. For the case, the number of mixer bus can be extended up
 * to 12.
 */
 ...
 * information in DSP controlled by register access
 * @data: Signal level meters. The mapping between position and input/output
 * channel is model-dependent.
 *
 * The structure expresses the part of DSP status for hardware meter. The u8
 * storage includes linear value for audio signal level between 0x00 and 0x7f.
 */
 ...
 * DSP controlled by register access.
 ...
 * @mixer.source.paired_balance: The L/R balance of paired source to mixer, only
 * for 4 pre and Audio Express.
 * @mixer.source.paired_width: The width of paired source to mixer, only for 4
 * pre and Audio Express.
 ...
 * @line_input.boost_flag: The flags of boost for line inputs, only for 828mk2
 * and Traveler.
 * @line_input.nominal_level_flag: The flags of nominal level for line inputs,
 * only for 828mk2 and Traveler.
 * @line_input.reserved: Padding for 32 bit alignment for future extension.
 * @input.gain_and_invert: The value including gain and invert for input, only
 * for Ultralite, 4 pre and Audio Express.
 * @input.flag: The flag of input; e.g. jack detection, phantom power, and pad,
 * only for Ultralite, 4 pre and Audio express.
 * @reserved: Padding so that the size of structure is kept to 512 byte, but for
 * future extension.
 *
 * The structure expresses the set of parameters for DSP controlled by register
 * access.
 */
...
/*
 * In below MOTU models, software is allowed to control their DSP by command in
 * frame of asynchronous transaction to 0x'ffff'0001'0000:
 *
 ...
 *  - Track 16
 *
 * On the other hand, the states of hardware meter is split into specific
 * messages included in the sequence of isochronous packet. ALSA firewire-motu
 * driver gathers the message and allow userspace application to read it via
 * ioctl.
 */
...
/**
 * struct snd_firewire_motu_command_dsp_meter - the container for meter
 * information in DSP controlled by command
 * @data: Signal level meters. The mapping between position and signal channel
 * is model-dependent.
 *
 * The structure expresses the part of DSP status for hardware meter. The 32 bit
 * storage is estimated to include IEEE 764 32 bit single precision floating
 * point (binary32) value. It is expected to be linear value (not logarithm) for
 * audio signal level between 0.0 and +1.0.
 */
