...
#define A3_EMU32IN(x)                                                    \
	(0x160 + (x)) /* x = 0x00 - 0x1f "EMU32_IN_00 - _1F" - Only when \
			 .device = 0x0008 */
#define A3_EMU32OUT(x)                                                    \
	(0x1E0 + (x)) /* x = 0x00 - 0x1f "EMU32_OUT_00 - _1F" - Only when \
			 .device = 0x0008 */
 ...
 * to TRAM and is	*/
/* decompressed back to 20 bits on a read.  There are a total of 160 locations,
 * the last 32	*/
...
struct snd_emu10k1_fx8010_pcm_rec {
	...
	unsigned short gpr_trigger;  /* GPR containing trigger (activate)
					information (host) */
	unsigned short gpr_running;  /* GPR containing info if PCM is running
					(FX8010) */
	unsigned char pad;	     /* reserved */
	unsigned char etram[32];     /* external TRAM address & data (one per
					channel) */
