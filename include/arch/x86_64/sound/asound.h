...
enum {
	SNDRV_PCM_SUBCLASS_GENERIC_MIX = 0, /* mono or stereo subdevices are
					       mixed together */
	SNDRV_PCM_SUBCLASS_MULTI_MIX,	    /* multichannel subdevices are mixed
					       together */
	/* Don't forget to change the following: */
 ...
 * information about the available bit count in most significant bit. It's for
 * the case of so-called 'left-justified' or `right-padding` sample which has
 * less width than 32 bit.
 */
...
#define SNDRV_PCM_FORMAT_FLOAT_LE                                           \
	((snd_pcm_format_t)14) /* 4-byte float, IEEE-754 32-bit, range -1.0 \
				  to 1.0 */
#define SNDRV_PCM_FORMAT_FLOAT_BE                                           \
	((snd_pcm_format_t)15) /* 4-byte float, IEEE-754 32-bit, range -1.0 \
				  to 1.0 */
...
struct snd_pcm_hw_params {
	...
	unsigned int msbits;	     /* R: used most significant bits (in sample
					bit-width) */
...
struct snd_pcm_sw_params {
	...
	snd_pcm_uframes_t xfer_align;	   /* obsolete: xfer size need to be a
					      multiple */
	snd_pcm_uframes_t start_threshold; /* min hw_avail frames for automatic
					      start */
	/*
...
enum {
	 ...
	 *  maps to wallclock/link time for HDAudio playback and DEFAULT/DMA
...
struct __snd_pcm_mmap_control64 {
	__pad_before_uframe __pad1;
	snd_pcm_uframes_t appl_ptr; /* RW: appl ptr (0...boundary-1) */
	__pad_before_uframe __pad2; // This should be __pad_after_uframe, but
				    // binary backwards compatibility
				    // constraints prevent a fix.
...
/*
 * This structure describes the userspace-driven timer. Such timers are purely
 * virtual, and can only be triggered from software (for instance, by userspace
 * application).
 */
struct snd_timer_uinfo {
	/* To pretend being a normal timer, we need to know the resolution in
	 * ns. */
	__u64 resolution;
	int fd;
	unsigned int id;
